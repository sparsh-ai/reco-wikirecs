...
def print_user_history(all_histories, user=None, userid=None):
    ...
    for i in range(len(edits))[::-1]:
        ...
        if row.ITEMID != last_page:
            print(" {} {}".format(str(dt.time()), row.title.strip()))
            last_page = row.ITEMID
...
def recall(test_set, recs, K=10, userid_subset=[]):
    ...
    hits = [
        ...
        for pageid, userid in zip(test_set.ITEMID, test_set.userid)
...
def prep_for_metrics(test_set, recs, K, userid_subset=None):
    ...
    y_true = [
        (p == r[:K]).astype(int)
        for p, r in zip(
            test_set[selected_rows].ITEMID.values, test_set[selected_rows].recs.values
        )
    ]
...
def display_recs_with_history(
    ...
    for u in userids:
        real_next = histories_test.loc[histories_test.userid == u].ITEMID.values[0]
        user_history = list(
...
def get_resurface_discovery(histories_train, histories_test):
    d = histories_train.merge(
        histories_test[["USERID", "ITEMID"]], on="USERID", suffixes=["", "TARGET"]
    )
    d.loc[:, "is_target"] = d.ITEMID == d.ITEMIDTARGET
